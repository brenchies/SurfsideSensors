...
Ezo_board EZO_EC_SENSOR_OBJECT = Ezo_board(EZO_EC_I2C_ADDRESS, "EC");
...
class ezo_ec_i2c{
    ...
    float readSensor(long delay_){
        EZO_EC_SENSOR_OBJECT.send_read_cmd();
        delay(delay_);
        EZO_EC_SENSOR_OBJECT.receive_read_cmd(); 
        float val = EZO_EC_SENSOR_OBJECT.get_last_received_reading();
        status = EZO_EC_SENSOR_OBJECT.get_error() == EZO_EC_SENSOR_OBJECT.SUCCESS? 1: -1;
        return val;
    }
