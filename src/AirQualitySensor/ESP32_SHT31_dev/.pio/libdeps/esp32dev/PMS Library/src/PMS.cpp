...
PMS::PMS(Stream &stream, String *name)
{
  this->_stream = &stream;
  this->_name = *name;
}
...
/**
 * @brief Sends a "read request" command to the PM sensor in order to
 * enable the sensor to begin transmitting measured data.
 *
 */
...
bool PMS::readUntil(DATA &data, int total_samples, uint16_t timeout)
{
  _data = &data;
  DATA temp_data;
